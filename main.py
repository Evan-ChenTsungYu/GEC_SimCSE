...
import torch
import json
from model import SimCSE_model
...
from pretrain_Enc import pretrain_simCSE
import os
def main():
    ...
    Model = SimCSE_model(parameter.model_name).to(DEVICE)
    Model.train()
    if args.TT == 'train':
        print(f' joint training type :{parameter.joint_training}')
        if os.path.exists(parameter.pretrain_save_path) == False:
            print('start pre_training on encoder')
            print(f'save file at {parameter.pretrain_save_path}')
            if os.path.exists(parameter.lang8_pretrained == True):
                print('load lang8 pretrained')
                Model.load_state_dict(torch.load(parameter.lang8_pretrained, map_location = DEVICE))

            pretrain_simCSE(Model, TT_DL, Val_DL, epochs = args.pretrain_epoch, result_save_path = parameter.pretrain_save_path, tokenizer = tokenizer, show_epoch_result = True, DEVICE = DEVICE, joint_training = parameter.joint_training)
        else :
